...
import {
  Box, Typography, Paper, Grid, Stepper, Step, StepLabel, Button, Chip, Divider,
  StepConnector, stepConnectorClasses, styled, StepIconProps, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Tabs, Tab,
  TextField, FormControl, FormLabel, RadioGroup, FormControlLabel, Radio, MenuItem, IconButton, InputAdornment, Select, Snackbar, Alert, AlertColor
} from '@mui/material';
...
// Custom Connector (Copied from NewRequestWizard)
...
// Custom Step Icon (Copied from NewRequestWizard)
...
export const RequestDetail: React.FC = () => {
  ...
  const fromProcessing = source === 'processing';
  const { requests, updateRequest } = useMockData();
  const { isTutorialOpen } = useTutorial();

  let request = requests.find(r => r.id === id);

  // If this is the "tutorial" draft request, use fake data
  // Note: In a real app, we'd fetch from ID 'draft_tutorial' properly or use a meaningful ID.
  // Here we just override based on ID match.
  if (id === 'draft_tutorial') {
    request = {
      id: 'draft_tutorial',
      title: 'Tutorial Draft Request',
      status: 'Draft',
      projectContact: { name: 'John Doe', email: 'john.doe@example.com' },
      country: 'Singapore',
      businessArea: 'Technology',
      lob: 'Group HQ',
      requestType: 'Talent Augmentation',
      description: 'This is a sample draft request for tutorial purposes.',
      engagementCriteria: 'Insourcing to VCC',
      resources: [],
      teamSize: 0,
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString()
    } as any; // Cast to any to satisfy type checking for partial data
  }
  ...
  const [snackbarMessage, setSnackbarMessage] = useState('Request updated successfully');
  ...
  // Redirect logic moved to TutorialOverlay to avoid race conditions
  ...
  const handleRemoveResource = (resourceId: string) => {
    setFormData(prev => {
      const updatedResources = prev.resources?.filter(r => r.id !== resourceId) || [];
      const newTeamSize = updatedResources.reduce((sum, r) => sum + (Number(r.quantity) || 0), 0);
      return {
        ...prev,
        resources: updatedResources,
  ...
  const handleUpdateRequest = () => {
    ...
    if (!formData.resources || formData.resources.length === 0) {
      ...
      setSnackbarMessage('You must add at least 1 resource before updating!');
    ...
    if (!formData.title?.trim()) {
      ...
      setSnackbarMessage('Please fill in Project Name / Title * before updating!');
      setSnackbarOpen(true);
      return;
    }
  ...
  const handleSubmitRequest = () => {
    ...
    // Success - Update request and change status to Evaluate
    const updatedRequest: Request = {
      ...formData as Request,
      status: 'Evaluate',
      updatedAt: new Date().toISOString()
  ...
  const handleConfirmEvaluateToDiscover = () => {
    if (!request) return;
    const updated = {
      ...request,
  ...
  const handleConfirmDiscoverToSignOff = () => {
    if (!request) return;
    const updated = {
      ...request,
  ...
  const handleCancelRequest = () => {
    ...
    const updated = {
      ...request,
      status: 'Closed' as const, // Closed maps to Cancelled in Processing view
      updatedAt: new Date().toISOString()
    } as typeof request;
    updateRequest(updated);
    navigate('/requests/processing');
  ...
  let activeStep = 0;
  if (request.status === 'Draft') activeStep = 0;
  else if (request.status === 'Evaluate') activeStep = 1;
  else if (request.status === 'Discover') activeStep = 2;
  else if (request.status === 'Sign-off') activeStep = 3;
  else if (request.status === 'Fulfill' || request.status === 'Closed') activeStep = 4;

  // Calculations for Invoice
  const totalMonthlyCost = (formData.resources || request.resources)?.reduce((acc, curr) => acc + (curr.subtotal || 0), 0) || 0;
  const totalWithMarkup = totalMonthlyCost;
  const monthlyGrandTotal = totalWithMarkup * 1.065;
  const durationMonths = parseInt((formData.duration || request.duration) || '0') || 0;
  const grandTotal = monthlyGrandTotal * durationMonths;

  // EDIT MODE: Render wizard interface for draft requests
  const [wizardStep, setWizardStep] = useState(1); // Default to Step 1 (Details) if editing draft

  // Navigation handlers
  const handleWizardNext = () => setWizardStep(1);
  const handleWizardBack = () => setWizardStep(0);
  ...
  if (request.status === 'Draft') {
    return (
      <Box sx={{ maxWidth: 900, mx: 'auto', py: 4 }}>
        ...
        {wizardStep === 0 ? (
          // STEP 0: Contact Info & Request Type
          <Box>
            <Box>
                ...
                <Box sx={{ minHeight: 200 }}>
                  <Box sx={{ mt: 2 }}>
                    <Grid container spacing={3}>
                      ...
                      {/* Engagement Criteria & File Upload */}
                      <Grid size={{ xs: 12, md: 6 }}>
            ...
            <Box>
              ...
              <Paper sx={{ p: 4, borderRadius: 2, boxShadow: '0px 4px 24px rgba(0,0,0,0.05)' }}>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                  ...
                  <Typography variant="body1">${totalWithMarkup.toLocaleString()}</Typography>
                </Box>
  ...
  // VIEW MODE: Render detail view for non-draft requests
  return (
    <Box>
      <Typography variant="h4" sx={{ mb: 5, fontWeight: 300 }}>Business Request Form</Typography>

      <Stepper activeStep={activeStep} alternativeLabel connector={<ColorlibConnector />} sx={{ mb: 6 }}>
        {STEPS.map((label) => (
          <Step key={label}>
            <StepLabel StepIconComponent={ColorlibStepIcon}>{label}</StepLabel>
          </Step>
      ...
      {fromProcessing && request.status === 'Evaluate' && (
        <Box sx={{ display: 'flex', gap: 2, mb: 2 }}>
          <Button
            variant="contained"
            color="primary"
            onClick={handleConfirmEvaluateToDiscover}
            sx={{ textTransform: 'none' }}
          >
            Confirm & Move to Discover
          </Button>
          <Button
            variant="outlined"
            color="error"
      ...
      {fromProcessing && request.status === 'Discover' && (
        <Box sx={{ display: 'flex', gap: 2, mb: 2 }}>
          <Button
            variant="contained"
            onClick={handleConfirmDiscoverToSignOff}
            sx={{
              textTransform: 'none',
              bgcolor: '#2e7d32',
      ...
      <Box sx={{ borderBottom: 1, borderColor: 'divider', mb: 2 }}>
        <Tabs value={currentTab} onChange={handleTabChange} aria-label="request detail tabs">
          <Tab label="Request Detail" sx={{ '&:focus': { outline: 'none' } }} />
          <Tab label="Discussion Log" sx={{ '&:focus': { outline: 'none' } }} />
        </Tabs>
      </Box>
      ...
      <Paper sx={{ p: 4, borderRadius: 2, boxShadow: '0px 4px 24px rgba(0,0,0,0.05)' }}>
        ...
        {currentTab === 0 && (
          <Box sx={{ border: '1px solid #e0e0e0', borderRadius: 2, p: 3 }}>
            {/* Section 1: Project Contact Info */}
            <Grid container spacing={1}>
              ...
              <Grid size={{ xs: 9 }}>
                <Typography>{request.projectContact?.name} ({request.projectContact?.email})</Typography>
              </Grid>

              {/* Project Name / Title */}
              <Grid size={{ xs: 3 }}>
                <Typography variant="subtitle2" sx={{ fontWeight: 'bold' }}>Project Name / Title:</Typography>
              </Grid>
              <Grid size={{ xs: 9 }}>
              ...
              <Grid size={{ xs: 3 }}>
                <Typography variant="subtitle2" sx={{ fontWeight: 'bold' }}>Project Description / Note:</Typography>
              </Grid>
              <Grid size={{ xs: 9 }}>
              ...
              <Grid size={{ xs: 3 }}>
                <Typography variant="subtitle2" sx={{ fontWeight: 'bold' }}>Project Contact Name:</Typography>
              </Grid>
              <Grid size={{ xs: 9 }}>
                <Typography>{request.projectContact?.name || '-'}</Typography>
              </Grid>

              {/* Project Contact Email */}
              <Grid size={{ xs: 3 }}>
                <Typography variant="subtitle2" sx={{ fontWeight: 'bold' }}>Project Contact Email:</Typography>
              </Grid>
              <Grid size={{ xs: 9 }}>
                <Typography>{request.projectContact?.email || '-'}</Typography>
              </Grid>

              {/* Business Area */}
              <Grid size={{ xs: 3 }}>
                <Typography variant="subtitle2" sx={{ fontWeight: 'bold' }}>Business Area:</Typography>
              </Grid>
              <Grid size={{ xs: 9 }}>
                <Typography>{request.businessArea || '-'}</Typography>
              </Grid>

              {/* LOB */}
              <Grid size={{ xs: 3 }}>
                <Typography variant="subtitle2" sx={{ fontWeight: 'bold' }}>LOB:</Typography>
              </Grid>
              <Grid size={{ xs: 9 }}>
                <Typography>{request.lob || '-'}</Typography>
              </Grid>

              {/* Country */}
              <Grid size={{ xs: 3 }}>
                <Typography variant="subtitle2" sx={{ fontWeight: 'bold' }}>Country:</Typography>
              </Grid>
              <Grid size={{ xs: 9 }}>
                <Typography>{request.country || '-'}</Typography>
              </Grid>

              {/* Engagement Criteria */}
              <Grid size={{ xs: 3 }}>
                <Typography variant="subtitle2" sx={{ fontWeight: 'bold' }}>Engagement Criteria:</Typography>
              </Grid>
              <Grid size={{ xs: 9 }}>
                <Typography>{request.engagementCriteria}</Typography>
              </Grid>

              {/* Request Type */}
              <Grid size={{ xs: 3 }}>
                <Typography variant="subtitle2" sx={{ fontWeight: 'bold' }}>Request Type:</Typography>
              </Grid>
              <Grid size={{ xs: 9 }}>
                <Typography>{request.requestType}</Typography>
              </Grid>

              {/* Supporting IRR Documents */}
              <Grid size={{ xs: 3 }}>
                <Typography variant="subtitle2" sx={{ fontWeight: 'bold' }}>Supporting IRR Documents:</Typography>
              </Grid>
              <Grid size={{ xs: 9 }}>
              ...
              {/* Expected Start Date & Duration */}
              <Grid size={{ xs: 3 }}>
                <Typography variant="subtitle2" sx={{ fontWeight: 'bold' }}>Expected Start Date:</Typography>
              </Grid>
              <Grid size={{ xs: 3 }}>
                <Typography>{request.startDate || '-'}</Typography>
              </Grid>
              <Grid size={{ xs: 3 }}>
                <Typography variant="subtitle2" sx={{ fontWeight: 'bold' }}>Duration:</Typography>
              </Grid>
              <Grid size={{ xs: 3 }}>
                <Typography>{request.duration || '-'}</Typography>
              </Grid>

              {/* Expected Lead Time & Team Size */}
              <Grid size={{ xs: 3 }}>
                <Typography variant="subtitle2" sx={{ fontWeight: 'bold' }}>Expected Lead Time:</Typography>
              </Grid>
              <Grid size={{ xs: 3 }}>
                <Typography>{request.expectedLeadTime}</Typography>
              </Grid>
              <Grid size={{ xs: 3 }}>
                <Typography variant="subtitle2" sx={{ fontWeight: 'bold' }}>Team Size:</Typography>
              </Grid>
              <Grid size={{ xs: 3 }}>
                <Typography>{request.teamSize}</Typography>
              </Grid>

            </Grid>

            <Paper variant="outlined" sx={{ mb: 4, mt: 4, overflowX: 'auto' }}>
                  ...
                  <TableBody>
                    {request.resources?.map((res, index) => (
                      <TableRow key={index}>
                        ...
                        <TableCell>{res.seniority}</TableCell>
                        <TableCell>{res.quantity}</TableCell>
                        <TableCell>${res.rate?.toLocaleString()}</TableCell>
                        <TableCell>{res.mandatorySkills}</TableCell>
                        <TableCell>{res.otherSkills}</TableCell>
                        <TableCell>${res.subtotal?.toLocaleString()}</TableCell>
                        <TableCell>{res.jobDescription}</TableCell>
                      </TableRow>
                    ))}
                    {(!request.resources || request.resources.length === 0) && (
                      <TableRow>
                        <TableCell colSpan={8} align="center">No resources added</TableCell>
                      </TableRow>
                    )}
                  </TableBody>
                </Table>
            ...
            <Paper variant="outlined" sx={{ p: 4, borderRadius: 2 }}>
              ...
              {/* Line 3 */}
              <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                <Typography variant="body1" sx={{ fontWeight: 'bold' }}>Grand total ({request.duration || '0 months'})</Typography>
                <Typography variant="body1">${Math.ceil(grandTotal).toLocaleString()}</Typography>
              </Box>
            </Paper>
